...
const CATEGORIES: IssueCategory[] = ['streetlight', 'road_damage', 'trash', 'water', 'electricity', 'flooding', 'abandoned_vehicle', 'safety', 'other'];
...
export default function CreateIssueScreen() {
  ...
  const stepTitles = [t.issueCreate.stepCategory, t.issueCreate.stepPhotos, t.issueCreate.stepLocation, t.issueCreate.stepDescription, t.issueCreate.stepReview];
  ...
  return (
    <KeyboardAvoidingView style={{ flex: 1, backgroundColor: theme.colors.background }} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
      ...
      <View style={[theme.row(), styles.progressRow, { paddingHorizontal: theme.spacing.md }]}>
        {STEPS.map((_, i) => (
          <View key={i} style={[styles.progressDot, { flex: 1, backgroundColor: i <= step ? theme.colors.primary : theme.colors.border }]} />
        ))}
      </View>
      <Text style={[theme.text('caption', theme.colors.textMuted), { paddingHorizontal: theme.spacing.md, marginTop: 8 }]}>{stepTitles[step]}</Text>
      ...
      <ScrollView contentContainerStyle={{ padding: theme.spacing.md }} keyboardShouldPersistTaps="handled">
          ...
          <View style={[theme.row(), styles.categoryGrid]}>
            {CATEGORIES.map((c) => (
              <Pressable
                ...
                style={[styles.categoryCell, { borderColor: category === c ? theme.colors.primary : theme.colors.border, backgroundColor: category === c ? theme.colors.successSurface : theme.colors.surface, borderRadius: theme.radii.md }]}
              >
                <Text style={theme.text('bodySmall', category === c ? theme.colors.primary : theme.colors.textPrimary)}>{t.issueCreate.categories[c]}</Text>
              </Pressable>
            ))}
          </View>
        ) : null}
        ...
        {step === 2 ? (
          <View>
            {neighborhood ? <MapPlaceholder centerLat={neighborhood.centerLat} centerLng={neighborhood.centerLng} pins={[{ id: 'x', lat: neighborhood.centerLat, lng: neighborhood.centerLng }]} height={220} /> : null}
        ...
        {step === 4 ? (
          <Card>
            ...
            {images.length > 0 ? <Text style={[theme.text('caption', theme.colors.textMuted), { marginTop: 8 }]}>{images.length} 📷</Text> : null}
          </Card>
        ) : null}
      </ScrollView>
...
const styles = StyleSheet.create({
  ...
  addImageBtn: { alignItems: 'center', gap: 8, alignSelf: 'flex-start', borderWidth: 1.5, borderStyle: 'dashed', borderRadius: 10, paddingVertical: 10, paddingHorizontal: 14, marginTop: 12 },
